...
export default function LiveAlertFeed({ onSelectWork, initialTier = 'all', activeRole = 'ministry' }) {
  ...
  const getTierBadge = (riskScore, tierName) => {
    ...
    // 1. Explicit Statutory / Model Tiers take primary precedence
    if (tierUpper === 'CRITICAL' || (!tierUpper && score100 >= 85.0)) {
      return (
    ...
    if (tierUpper === 'HIGH' || (!tierUpper && score100 >= 61.4)) {
      return (
    ...
    if (tierUpper === 'MEDIUM' || tierUpper === 'MED' || (!tierUpper && score100 >= 48.4)) {
      return (
        <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold font-mono bg-violet-500/15 text-violet-300 border border-violet-500/30">
          <span className="w-2 h-2 rounded-full bg-violet-400 mr-1.5" />
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {[
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-xl text-xs font-semibold transition-all cursor-pointer ${tier === t.id
                    ? t.id === 'critical'
          ...
          <div className="relative z-30" ref={anomalyDropdownRef}>
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-xl text-xs font-semibold flex items-center gap-2 transition-all cursor-pointer border ${triggerFilter !== 'all'
                  ...
                  {ANOMALY_TYPES.map((trig) => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-full px-2.5 py-2 rounded-lg text-xs font-medium flex items-center justify-between transition-colors text-left cursor-pointer ${isSelected
              ...
              {loading ? (
                ...
                flags.map((work) => {
                  ...
                  return (
                    <tr
                      key={work.work_id}
                      className={`hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group border-b border-slate-100 dark:border-white/[0.03] ${isCritical ? 'bg-rose-50/60 dark:bg-rose-950/10' : ''
                        }`}
                    >
                      {/* Work ID & Title */}
                      ...
                      <td className="py-4 px-4">
                        <div>
                          {getTierBadge(score100, tierUpper)}
                        </div>
                        <div className="w-28 bg-slate-800 h-2 rounded-full overflow-hidden mt-2">
                          <div
                            className={`h-full rounded-full transition-all duration-300 ${isCritical ? 'bg-rose-500' :
                                isHigh ? 'bg-amber-400' :
                                  isMed ? 'bg-violet-400' :
                                    'bg-emerald-400'
                              }`}
                        ...
                        {work.completion_probability !== undefined && (
                          <div className="text-xs font-mono text-slate-300 mt-1.5 flex items-center gap-1" title="Logistic Regression Completion Likelihood">
                            <span>Comp:</span>
                            <span className={`font-semibold ${Number(work.completion_probability) >= 0.70 ? 'text-emerald-400' :
