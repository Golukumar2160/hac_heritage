...
export default function SignupForm({ 
  ...
  const [form, setForm] = useState({
    ...
    ida: (authOptions.districts_by_state?.['Uttar Pradesh'] || [])[0] || 'PILIBHIT(DISTRICT MAGISTRAE PILIBHIT_IDA)',
    mp_name: '',
    house: ''
  });
  ...
  const handleStateChange = (newState) => {
    const districts = (authOptions.districts_by_state && authOptions.districts_by_state[newState]) || [];
    setForm(prev => ({
      ...prev,
      state: newState,
      ida: districts[0] || ''
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        designation: selectedRole === 'citizen' ? 'Jan-Drishti Public Watchdog' : (form.designation?.trim() || currentRoleMeta.title),
        state: selectedRole !== 'ministry' ? (form.state || 'Uttar Pradesh') : undefined,
        ida: (selectedRole === 'district' || selectedRole === 'citizen') ? (form.ida || 'PILIBHIT') : undefined,
        mp_name: selectedRole === 'mp' ? (form.mp_name || form.name.trim()) : undefined
      };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      {selectedRole !== 'ministry' && (
        <div className="p-4 rounded-2xl bg-[#040714]/90 border border-violet-500/20 space-y-3.5">
          ...
          {(selectedRole === 'state' || selectedRole === 'district' || selectedRole === 'citizen') && (
            <div>
              <label className="block text-xs sm:text-sm font-bold text-slate-200 mb-1.5 font-mono">
                {selectedRole === 'citizen' ? 'Select Your State / Union Territory' : 'Designated State / Union Territory'} {authOptions.states?.length ? `(${authOptions.states.length} States & UTs)` : ''} <span className="text-rose-400">*</span>
              </label>
              <select
                value={form.state}
              ...
              >
                ...
                {(authOptions.states || []).filter(s => s !== 'Uttar Pradesh').map(st => {
                  const dCount = (authOptions.districts_by_state && authOptions.districts_by_state[st] || []).length;
                  return (
                    <option key={st} value={st}>
                      {st} {dCount ? `(${dCount} Districts)` : ''}
                    </option>
                  );
                })}
              </select>
          ...
          {(selectedRole === 'district' || selectedRole === 'citizen') && (
            <div>
              <label className="block text-xs sm:text-sm font-bold text-slate-200 mb-1.5 font-mono">
                {selectedRole === 'citizen' ? 'Select Your District to Monitor' : 'Implementing District Authority (IDA)'} <span className="text-rose-400">*</span>
              </label>
              <select
                value={form.ida}
              ...
              >
                {currentDistricts.length > 0 ? (
                  ...
                  <option value="PILIBHIT(DISTRICT MAGISTRAE PILIBHIT_IDA)">PILIBHIT (DISTRICT MAGISTRATE PILIBHIT)</option>
                )}
              </select>
            </div>
          )}
      ...
      {/* Designation (Optional for official tiers) */}
      {selectedRole !== 'citizen' && (
        <div>
          ...
          <input
            ...
            placeholder={currentRoleMeta.title}
            className="w-full px-4 py-3 rounded-xl bg-[#040714] border border-slate-700/80 text-white text-sm sm:text-base focus:outline-none focus:border-violet-400 focus:ring-2 focus:ring-violet-500/20 font-sans placeholder-slate-500"
          />
        </div>
      )}
